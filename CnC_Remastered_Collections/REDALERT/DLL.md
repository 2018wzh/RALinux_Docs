...
```cpp
 ...
 void __cdecl CNC_Restore_Carryover_Objects(const CarryoverObjectStruct* objects);
...
calls DLL_Startup
### DLL_Startup
The original RA95's WinMain function with some modifications.
- 178:Receive instance and command line from launcher
- 196:Deletes check of running
- 304:Adds quote check
- 321:Removes remembering drives
- 429:Removes check of timer
- 442:The editor needs to load the Red Alert ini file from a different location than the real game. - 7/18/2019 JAS 
- 484:Removes storage space check
- 575:Replace init of GBC with new versions
- 639:Change default size.
- 662:Changes for editor mode, and removes CD check.
- 740:Exits when running as DLL
## 